...
export default function CheckoutPage() {
    ...
    <button
      ...
      onClick={() => {
        ...
        const found = promos.find(
          (p) => String(p.code).trim().toUpperCase() === code && p.active
        );
        ...
        // базовая проверка min_total сразу
        if (sum < (found.min_total || 0)) {
          setPromoApplied(null);
          setPromoError(`Минимальная сумма для промокода: ${found.min_total} тг`);
          return;
        }
  ...
  {promoApplied && (
    <div className="hint">
      Применён: <b>{promoApplied.code}</b> (скидка {promoApplied.type === "percent" ? `${promoApplied.value}%` : `${promoApplied.value} тг`})
      <button
        type="button"
        className="clearBtn"
        style={{ marginLeft: 10 }}
  ...
  {promoError && <div className="hint" style={{ color: "#ff6b6b" }}>{promoError}</div>}
</div>
...
useEffect(() => {
  ...
  const [cashFrom, setCashFrom] = useState(""); // сдача с суммы
...
const discount = useMemo(() => {
  ...
  const payText = useMemo(() => {
    if (pay === "cash") return "Наличные";
    if (pay === "kaspi") return "Kaspi.kz";
    if (pay === "halyk") return "Halyk";
  ...
  const waLink = useMemo(() => {
    ...
    items.forEach((i, idx) => {
      lines.push(
        `${idx + 1}) ${i.title} — ${i.qty} шт — ${itemPrice(i) * i.qty} тг`
      );
    });

    lines.push("");
    lines.push(`Товары: ${sum} тг`);
    lines.push(`Доставка: ${delivery === 0 ? (sum >= FREE_FROM ? "Бесплатно" : "0 тг") : `${delivery} тг`}`);
          ...
          <div className="form">
            ...
            <div className="payBox">
              ...
              <div className="payRow">
                <button
                  type="button"
                  className={`payBtn ${pay === "cash" ? "active" : ""}`}
                  onClick={() => setPay("cash")}
                >
                  💵 Наличные
                </button>
                <button
                  type="button"
                  className={`payBtn ${pay === "kaspi" ? "active" : ""}`}
                  onClick={() => setPay("kaspi")}
                >
                  🟣 Kaspi
                </button>
                <button
                  type="button"
                  className={`payBtn ${pay === "halyk" ? "active" : ""}`}
                  onClick={() => setPay("halyk")}
                >
                  🟢 Halyk
                </button>
                <button
                  type="button"
                  className={`payBtn ${pay === "card" ? "active" : ""}`}
                  onClick={() => setPay("card")}
                >
                  💳 Карта
                </button>
